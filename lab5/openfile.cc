...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    if ((position + numBytes) > fileLength) {
        ...
        bool success = hdr->Allocate(freeMap, fileLength, incrementBytes);
    ...
    // Preserve existing bytes in old sectors. Newly allocated sectors stay
    // zero-filled until the caller's bytes are copied in.
    for (i = firstSector; i <= lastSector; i++) {
        int sectorStart = i * SectorSize;
        if (sectorStart < fileLength)
            synchDisk->ReadSector(hdr->ByteToSector(sectorStart),
                                  &buf[(i - firstSector) * SectorSize]);
    }

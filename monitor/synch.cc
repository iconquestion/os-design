...
void
Condition_H::Wait(Semaphore *mutex, Semaphore *next, int *next_countPtr) {
    IntStatus oldLevel = interrupt->SetLevel(IntOff);
...
void
Condition_H::Signal(Semaphore *next, int *next_countPtr) {
    IntStatus oldLevel = interrupt->SetLevel(IntOff);
    if (count > 0) {
        (*next_countPtr)++;
        sem->V();
...
void
Condition_H::Broadcast(Semaphore *next, int *next_countPtr) {
    // not implemented
}
